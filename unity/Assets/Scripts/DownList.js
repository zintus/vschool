...
var s1 = transform.parent.transform.Find("Plane").GetComponent(NameMovie);
	...
	list1.GetComponent(TextMesh).text = s1.hdr[s1.n1+1]; 
	list2.GetComponent(TextMesh).text = s1.hdr[s1.n1+2];
	list3.GetComponent(TextMesh).text = s1.hdr[s1.n1+3];
	list4.GetComponent(TextMesh).text = s1.hdr[s1.n1+4];
	s1.n1=s1.n1+1;
	//print(s1.n1);
	}
}